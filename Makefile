...
PROJECT = testcontainers-orm
PACKAGE = testcontainers_orm
DEV ?= 1
...
install:
	make prepare
	${POETRY} install -vvv --remove-untracked --extras "sqlalchemy tortoise redis alembic" `if [ "${DEV}" = "0" ]; then echo "--no-dev"; fi`
